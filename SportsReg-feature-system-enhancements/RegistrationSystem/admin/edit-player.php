<?php
  ...
  if (isset($_POST['updateplayer'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
      ...
      $player_id_query = mysqli_query($db_con, "SELECT `player_id` FROM `players` WHERE `id` = $id");
      $player_id_row = mysqli_fetch_assoc($player_id_query);
    ...
    if (mysqli_query($db_con, $query)) {
      ...
      header('Location: index.php?page=all-players&edit=success');
    } else {
      header('Location: index.php?page=all-players&edit=error');
    }
  }
?>
