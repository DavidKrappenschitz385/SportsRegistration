		...
		if(count($input_arr)==0){
			...
			if (mysqli_num_rows($result)==1) {
				...
				if (password_verify($password, $row['password'])) {
					...
					if ($row['role'] == 'admin') {
						header('Location: index.php');
					} elseif ($row['role'] == 'default') {
						header('Location: ../request-role.php');
					} elseif (($row['role'] == 'player' || $row['role'] == 'coach') && $row['status'] == 'approved') {
						header('Location: index.php');
					} else {
						$worngpass= "Your account is not approved yet.";
					}
				}else{
